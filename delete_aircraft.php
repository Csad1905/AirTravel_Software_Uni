...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Collect aircraft number from the form
    $aircraftNumber = $_POST["deleteAircraftId"];

    // SQL query to find all flights associated with the aircraft number
    $sql_flights = "SELECT flight_no FROM flights WHERE aircraft_id = '$aircraftNumber'";
    ...
    if ($result_flights->num_rows > 0) {
        ...
        echo "No flights associated with the provided aircraft number!";
    }

    // SQL query to delete the aircraft
    $sql_delete_aircraft = "DELETE FROM aircraft WHERE aircraft_id = '$aircraftNumber'";
    if ($conn->query($sql_delete_aircraft) !== TRUE) {
        echo "Error deleting aircraft: " . $conn->error;
    } else {
        echo "Aircraft with number $aircraftNumber deleted successfully!";
    }
}
